...
labels = ['Number of bedrooms', 'Livable surface',
          'Balcony', 'Surface bedroom 1', 'Furnished', 'Surface of living-room', 'Cellar', 'Surface kitchen', 'Number of facades', 'Terrace', 'Surface terrace',
       'Garden', 'Surface garden', 'Garage',
...
def cleaning_by_key(data, key, details):
    ...
    if data:
        try:
            ...
            print(f"The data for {key} ({data}) is not in the right format (expected {type_})")
    else:
         return default
...
def cleaning(data):
    ...
    undefined_keys = [key for key in label_details.keys() if key not in data.keys()]
    for key in undefined_keys:
        data[key] = cleaning_by_key(None, key, label_details[key])

one_hot_labels = [ 'Kitchen equipment_Fully equipped', 'Kitchen equipment_Not equipped', 'Kitchen equipment_Partially equipped',
       'Kitchen equipment_Super equipped', 'State of the property_Excellent',
       'State of the property_Fully renovated', 'State of the property_New',
       'State of the property_Normal', 'State of the property_To be renovated',
       'Type of property_flat', 'Type of property_house']
